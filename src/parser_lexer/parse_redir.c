...
/*   Updated: 2023/06/28 17:09:36 by vtrevisa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../Include/minishell.h"

static void	if_2more(char **parsed, t_data *data, t_parser *p)
{
	p->redir_type = 1;
	p->index2 = 0;
...
static void	if_2less(char **parsed, t_data *data, t_parser *p)
{
	p->redir_type = 1;
	p->index2 = 1;
	create_redir_str(parsed, p, data, "<<");
...
static void	if_1less(char **parsed, t_data *data, t_parser *p)
{
	p->redir_type = 0;
	p->index2 = 1;
	create_redir_str(parsed, p, data, "<");
...
char	**parse_redir(char **parsed, t_data *data, int index)
{
	...
	while (parsed[++p.i])
	{
		if (ft_strncmp(parsed[p.i], ">>", 2) == 0)
			if_2more(parsed, data, &p);
		else if (ft_strncmp(parsed[p.i], ">", 1) == 0)
			if_1more(parsed, data, &p);
		else if (ft_strncmp(parsed[p.i], "<<", 2) == 0)
			if_2less(parsed, data, &p);
		else if (ft_strncmp(parsed[p.i], "<", 1) == 0)
			if_1less(parsed, data, &p);
	}
	return (parsed);
}
